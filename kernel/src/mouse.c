...
void mouse_irq_handler(void){packet[index++]=inb(0x60);if(index<3)return;index=0;if(!(packet[0]&8))return;x+=(int8_t)packet[1];y-=(int8_t)packet[2];if(x<0)x=0;if(y<0)y=0;if(x>=GFX_WIDTH)x=GFX_WIDTH-1;if(y>=GFX_HEIGHT)y=GFX_HEIGHT-1;}
