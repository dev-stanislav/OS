...
void users_init(void) { user_count=0;current_user=0;(void)fs_create("/system",FS_DIR,fs_root());load_users();if(!user_count){add_loaded("root",1);save_users();}int root=find_user("root");current_user=root>=0?(uint8_t)root:0; }
...
uint8_t users_login(const char *name) { int index=find_user(name);if(index<0){vga_write("user not found\n",VGA_COLOR_LIGHT_RED,VGA_COLOR_BLACK);return 0;}current_user=(uint8_t)index;vga_write("logged in as ",VGA_COLOR_LIGHT_GREEN,VGA_COLOR_BLACK);vga_write(users[current_user].name,VGA_COLOR_LIGHT_GREEN,VGA_COLOR_BLACK);vga_write("\n",VGA_COLOR_LIGHT_GREEN,VGA_COLOR_BLACK);return 1; }
