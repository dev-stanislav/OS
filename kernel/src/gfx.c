...
static uint32_t backbuffer[GFX_WIDTH * GFX_HEIGHT];
...
void gfx_rect(int x,int y,int w,int h,uint32_t color){uint32_t*t=target();if(x<0){w+=x;x=0;}if(y<0){h+=y;y=0;}if(x+w>GFX_WIDTH)w=GFX_WIDTH-x;if(y+h>GFX_HEIGHT)h=GFX_HEIGHT-y;if(w<=0||h<=0)return;for(int yy=0;yy<h;yy++)for(int xx=0;xx<w;xx++)t[(y+yy)*GFX_WIDTH+x+xx]=color;}
...
void gfx_pixel(int x,int y,uint32_t color){uint32_t*t=target();if(x>=0&&x<GFX_WIDTH&&y>=0&&y<GFX_HEIGHT)t[y*GFX_WIDTH+x]=color;}
